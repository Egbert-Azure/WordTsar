 ...
 *
 * @brief Linux spell checker using hunspell (legacy wxWidgets code).
 *
 * Implements the cHunspellSpellCheck class, which integrates the hunspell
 * library to provide document-level spell checking on Linux. Walks paragraphs
 * from the current position, tokenizes text, checks each word against the
 * en_US dictionary plus a user custom dictionary, and offers suggestions
 * for misspelled words.
 *
 * @section clinuxspellcheck_status Current Status
 * This file contains commented-out wxWidgets code and is NOT currently
 * compiled. The active spell checker is the platform-independent
 * cSpellChecker in src/core/spellcheck/, and the active spell check UI
 * is cSpellCheck in cspellcheck.cpp (Qt-based dialog).
 *
 * @author Gerald Brandt
 * @copyright GNU Affero General Public License v3.0
 *
 * @see cHunspellSpellCheck Legacy hunspell integration class
 * @see eWordRet Word check result enumeration
 * @see cSpellChecker Active platform spell checker (core)
 * @see cSpellCheck Active spell check dialog (GUI)
 */
...
cHunspellSpellCheck::~cHunspellSpellCheck(void)
{
